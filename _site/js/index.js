...
function getColor(d){
  ...
  if (language == null){
    ...
    c = colors[language]["color"]
    return c
  }
};

d3.json("https://api.github.com/users/cqkh42/repos").then(data => doEverything(data));

function doEverything(data){
  d = data;
  
  data = data.sort(function(x, y){return d3.ascending(String(x.language), String(y.language))});
  console.log(data.map(d => d["language"]))
 
  var svg = d3.select("#projects-image")
    .append("svg")
    .classed("proj-svg", true)
    .style("width", "100%")
    .style("height", "400px")
...
//  .style("font-size", "48px")
  ...
  entAp = dat
    .enter()
    .append("a")
    .attr("xlink:href", d => d["data"]["html_url"])
    .attr("target", "_blank")
    .on("mouseover", changeName)
    .append("circle")
